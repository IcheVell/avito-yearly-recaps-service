...
Получить развлекательное предсказание на следующий год в стиле печенья с предсказанием.
...
- `text` — короткий развлекательный текст;
- `type` — тип ответа, всегда `fortune`.

Если GigaChat API недоступен или не настроен, backend возвращает безопасный локальный fallback-текст в том же формате.
